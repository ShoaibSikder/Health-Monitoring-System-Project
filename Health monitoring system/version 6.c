...
void seeyourrecord(); // done by Arnob

void registe();              // done by shoaib
void login();                // done by shoaib
void Addrecord();            // done by shoaib
void generatePrescription(); // done by shoaib
void detectDisease();        // done by shoaib
void resourceAllocation();   // done by shoaib
void viewDoctors();          // done by shoaib
void addDoctor();            // done by shoaib
void otherResources();       // done by shoaib

void Title();        // done by forhad
...
void registe() {
    ...
    info_file = fopen("information.txt", "a"); // Open file for user info
    ...
    p.patientID = patientID; // Store the chosen hospital ID with the patient
...
void detectDisease() {
    ...
    info_file = fopen("information.txt", "a"); // Open file for user information
...
void generatePrescription() {
    ...
    FILE *info_file;
    info_file = fopen("information.txt", "r+");
    ...
    while(fscanf(info_file,"%d %s %s %s %s %s %s %s\n", &p.patientID, p.firstname, p.lastname, p.age, p.gender, p.Contactno, p.disease,p.date) != EOF) {
        ...
        if (p.patientID == patientid) {
            ...
            printf("______________________________________________________________________________________________\n");
            char date[50];
            printf("Enter next visit date(d-m-y) : ");
            scanf("%s", date);
            fprintf(info_file, " %s\n", date);
            // Notification System
            time(&rawtime);
            ...
            if (strcmp(date, current_date) == 0) {
                // Create a file with patient's name
                char filename[100];
